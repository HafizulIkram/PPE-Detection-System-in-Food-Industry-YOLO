...
class PDF(FPDF):
    ...
    def card(self, title, img_path, text):
        self.add_page()
...
def create_pdf_report(class_counts, status_counts, analysis_paragraph, start_date, end_date, file_path):
    ...
    class_analysis = analysis_paragraph.split('\n\n')[1]
    status_analysis = analysis_paragraph.split('\n\n')[2]
    ...
    pdf.cell(200, 10, txt=f"Report from {start_date} to {end_date}", ln=True, align='C')
    
    # Add class bar chart and analysis
    pdf.ln(10)
    pdf.set_font("Arial", 'B', 16)
    pdf.cell(0, 10, "Class Distribution", ln=True, align='C')
    pdf.image(class_chart_path, x=10, y=pdf.get_y() + 10, w=180)
    pdf.ln(100)
    pdf.set_font("Arial", size=12)
    pdf.multi_cell(0, 10, class_analysis)
    ...
    pdf.ln(10)
    pdf.set_font("Arial", 'B', 16)
    pdf.cell(0, 10, "Completion Status Distribution", ln=True, align='C')
    pdf.image(status_chart_path, x=10, y=pdf.get_y() + 10, w=180)
    pdf.ln(100)
    pdf.set_font("Arial", size=12)
    pdf.multi_cell(0, 10, status_analysis)
